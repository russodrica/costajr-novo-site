...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // Documentos marcados como SIGILOSOS (item ou banco inteiro) são barrados AQUI, no
    // servidor — não basta sumirem da tela: mesmo que o pedido venha com o id deles, não vão.
    ...
    if (exIds.length) {
      ...
      for (const r of (data || []) as any[]) {
        if (linhaEhSigilosa(r, sigilosos)) { bloqueados++; continue; }
