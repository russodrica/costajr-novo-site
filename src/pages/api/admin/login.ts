...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    // "fornecedor" = usuário EXTERNO: loga no painel, mas o acesso é deny-by-default
    // (só Documentos da Empresa + Extratos Bancários, somente leitura — permissoes.ts).
    if (!["admin", "manutencao_operacao", "manutencao_administrativo", "operacional", "rh", "financeiro", "comercial", "juridico", "coordenador", "fornecedor"].includes(perfil.role)) {
